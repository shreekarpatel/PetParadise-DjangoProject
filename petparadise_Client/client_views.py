...
from petparadise_Admin.models import customer, area, category, sub_category, product, feedback, cart, wishlist, order, \
    order_item, shelter_home, doctor, appointment, gallery, doctor_slots
from petparadise_Admin.forms import CustForm, areaform, categoryform, subcategoryform, productform
...
from petparadise_Client.cart import client_Cart
...
def c_appointment(request, id):
    if "client_id" in request.session:
        s = doctor_slots.objects.filter(doctor_id=id)
        c = doctor_slots.objects.filter(doctor_id=id, s_status=3).count()
        print("--- count ---", c)
        if request.method == "POST":
            try:
                user = request.session['client_id']

                a_date = request.POST.get("appointment_date")
                a_time = request.POST.get("slot_id")
                slo = doctor_slots.objects.get(slot_id=a_time)
                appoint = appointment(c_id_id=user, doctor_id_id=id, appointment_date=a_date,
                                      appointment_time=slo.doctor_time,
    ...
    return render(request, "c_appointment.html", {'s': s, 'did': id,'count' : c})
...
def visitor_only(request):
    return render(request, "visitoronly.html")
