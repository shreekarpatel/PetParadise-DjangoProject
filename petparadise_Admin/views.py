...
from petparadise_Admin.forms import CustForm, orderform, shelterform, doctorform, categoryform, subcategoryform, \
    productform, orderitemform, galleryform, feedbackform, appointmentform, areaform
from petparadise_Admin.models import customer, shelter_home, doctor, category, sub_category, product, order, order_item, \
    gallery, feedback, appointment, area, cart, wishlist

# <--------Data Show of Data Dictionary------->
from petparadise_Admin.function import handle_upload_file
...
def destory_category(request, id):
    custo = doctor.objects.get(category_id=id)
    custo.delete()
...
def show_admindashboard(request):
    ...
    app = appointment.objects.all()
    return render(request, "admin_dashboard.html", {'orderlist': dash,'products': p,'customers': c, 'orders': o, 'appointmentss': a, 'appointments': app})
...
class ProjectChart(APIView):
    ...
    def get(self, request, format=None):
        ...
        cursor.execute(
            '''SELECT (select product_name from product where product_id = i.product_id_id) as name , sum(i.product_price) as amount FROM order_item i join product p where p.product_id = i.product_id_id 
